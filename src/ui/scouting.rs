use eframe::egui;
use crate::game::{GameState, Player, AgentRole};
use rand::Rng;
...
impl ScoutingScreen {
    ...
    pub fn show(&mut self, ctx: &egui::Context, game_state: &mut GameState) {
                        ...
                        if self.scouted_players.is_empty() {
                            ...
                            egui::ScrollArea::vertical().show(ui, |ui| {
                                for player in &self.scouted_players {
                                    ...
                                    if ui.small_button("Sign Player").clicked() && game_state.budget >= player.market_value {
        ...
        // Handle actions outside UI closure
    ...
    fn scout_new_players(&mut self) {
        if self.scouting_budget >= 10_000 {
            ...
            // Generate 3-5 new players
            let mut rng = rand::thread_rng();
            let num_players = rng.gen_range(3..=5);
            
            for i in 0..num_players {
                let player = Player::generate_random(format!("Scout{}{}", 
